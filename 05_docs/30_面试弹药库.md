...
- 【坑·修复收尾 2026-08-17】上述 403 的放大器是 BasicConsume 默认 exclusive=true：独占位让"第二个消费者实例"永远 403，堵死按积压横向扩容的路。已显式传 false（consumer/main.cpp）——多实例时 broker 轮询分发。讲故事可升级为完整弧线：踩坑（僵尸 403）→ 根因（exclusive 默认值）→ 修复（_exit + 非独占）→ 设计原则（消费端要可扩容，独占位与它矛盾）。
...
- 【坑】中文编辑距离按字节算：一字 3 字节，"肺节结"→"肺结节"两个字都不同，按字节距离是 6（一字之差才是 3，如"骨裂"→"骨折"）——远超 max_dist=2 → 纠错空结果。改用 utfcpp 切码点后距离 2 正常召回——多字节文本的字节操作都要过一遍这个检查。（2026-08-17 校准：原记"距离 3"有误，已用 DP 实算复核。）
...
5. **pkill 自杀坑再现**：容器内脚本 `pkill -f "bin/xxx"` 会匹配到自己所在的 `bash -c` 命令行（内含模式串）导致 SIGTERM 自杀——必须 `$` 锚定（日常使用说明第七节早有记载，这次又被咬了一口）。
...
- [x] 2026-08-17：面试官问题全景库建成 → `20_面试题库.md`（18 个板块 + 高危 Top 15 + Gap 清单；本文件是"答案库"，那份是"题库"，配套使用）
- [x] 2026-08-17：RedisCache 线程安全已修（mutex 成员 + get/setex 上锁；"为什么锁不用 thread_local"的取舍写在头文件注释，可当讲点；待容器内编译验证）
- [x] 2026-08-17 深夜：面试包全量产出——答案库 16 文件（答案库/，约 2900 行口语稿，覆盖题库全部约 250 题）+ 手撕参考集（面试工具包/hands_on/，4 个 cpp 本机编译自测全绿）+ 统一口径与生产数字（面试工具包/00）+ 简历修订稿（简历/）。**数字未人工校对**，流程见 面试工具包/明早校对清单.md
- [x] 2026-08-17 深夜代码修正（均待容器编译验证）：consumer BasicConsume 显式 exclusive=false（多消费者扩容的根基）；RedisCache 补 redisSetTimeout；RIS IO 线程数可配置（RIS_IO_THREADS，默认 4）；TlvCodec.h 注释与实现对齐
